...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    counterAge = UserData.getAge() ?? 20;
    counterWeight = UserData.getWeight() ?? 59;
    counterHeight = UserData.getHeight() ?? 175;
    gender = UserData.getGender() ?? false;
    userBmi = UserData.getUserBmiList() ?? [];
    userDate = UserData.getUserDate() ?? [];
  }
        ...
        child: Column(
          children: [
            ...
            GestureDetector(
              ...
              onTap: () async {
                await UserData.setAge(counterAge);
                await UserData.setHeight(counterHeight);
                await UserData.setWeight(counterWeight);
                ...
                : Column(
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          setState(() {
                            UserData.clearDataBMi();
                            UserData.clearDataDate();
                            userBmi = UserData.getUserBmiList() ?? [];
                            userDate = UserData.getUserDate() ?? [];
                          });
                        },
